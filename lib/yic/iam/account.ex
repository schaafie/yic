defmodule Yic.Iam.Account do
  ...
  def changeset(account, attrs) do
    account
    |> cast(attrs, [:login, :user_id, :confirmed_at])
    |> validate_required([:login, :user_id])
  end
  ...
  defp validate_login(changeset) do
    ...
    |> validate_length(:login, max: 16)
    |> unsafe_validate_unique(:login, Yic.Repo)
  ...
  #   |> unsafe_validate_unique(:email, Yic.Repo)
  ...
  """
  def valid_password?(%Yic.Iam.Account{hashed_password: hashed_password}, password)
