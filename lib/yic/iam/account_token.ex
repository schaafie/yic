defmodule Yic.Iam.AccountToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "accounts_tokens" do
    ...
    belongs_to :account, Yic.Iam.Account
  ...
  def build_session_token(account) do
    ...
    {token, %Yic.Iam.AccountToken{token: token, context: "session", account_id: account.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Yic.Iam.AccountToken, where: [token: ^token, context: ^context]
  end
  ...
  def account_and_contexts_query(account, :all) do
    from t in Yic.Iam.AccountToken, where: t.account_id == ^account.id
  end

  def account_and_contexts_query(account, [_ | _] = contexts) do
    from t in Yic.Iam.AccountToken, where: t.account_id == ^account.id and t.context in ^contexts
  end
end
