defmodule YicWeb.Html.Iam.AccountSessionControllerTest do
  ...
  describe "GET /html/iam/accounts/log_in" do
    test "renders log in page", %{conn: conn} do
      conn = get(conn, Routes.html_iam_account_session_path(conn, :new))
    ...
    test "redirects if already logged in", %{conn: conn, account: account} do
      conn = conn |> log_in_account(account) |> get(Routes.html_iam_account_session_path(conn, :new))
      assert redirected_to(conn) == "/"
    end
  end

  describe "POST /html/iam/accounts/log_in" do
    test "logs the account in", %{conn: conn, account: account} do
      conn =
        post(conn, Routes.html_iam_account_session_path(conn, :create), %{
    ...
    test "logs the account in with remember me", %{conn: conn, account: account} do
      ...
      assert conn.resp_cookies["_yic_web_account_remember_me"]
      assert redirected_to(conn) == "/"
    end
    ...
    test "logs the account in with return to", %{conn: conn, account: account} do
      conn =
        ...
        |> post(Routes.html_iam_account_session_path(conn, :create), %{
          "account" => %{
            "email" => account.email,
            "password" => valid_account_password()
          }
  ...
  describe "DELETE /html/iam/accounts/log_out" do
    test "logs the account out", %{conn: conn, account: account} do
      conn = conn |> log_in_account(account) |> delete(Routes.html_iam_account_session_path(conn, :delete))
      assert redirected_to(conn) == "/"
    ...
    test "succeeds even if the account is not logged in", %{conn: conn} do
      conn = delete(conn, Routes.html_iam_account_session_path(conn, :delete))
      assert redirected_to(conn) == "/"
      refute get_session(conn, :account_token)
      assert get_flash(conn, :info) =~ "Logged out successfully"
    end
