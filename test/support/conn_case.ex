defmodule YicWeb.ConnCase do
  ...
  by setting `use YicWeb.ConnCase, async: true`, although
  ...
  using do
    quote do
      ...
      import YicWeb.ConnCase

      alias YicWeb.Router.Helpers, as: Routes

      # The default endpoint for testing
      @endpoint YicWeb.Endpoint
    end
  end

  setup tags do
    pid = Ecto.Adapters.SQL.Sandbox.start_owner!(Yic.Repo, shared: not tags[:async])
  ...
  def register_and_log_in_account(%{conn: conn}) do
    account = Yic.IamFixtures.account_fixture()
  ...
  def log_in_account(conn, account) do
    token = Yic.Iam.generate_account_session_token(account)
